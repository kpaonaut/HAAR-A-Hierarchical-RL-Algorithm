...
# SNN policy settings
latent_dim = 1   # dim of the latent variables in the SNN
...
for height in np.arange(0.14, 0.2, 0.02):
    env = normalize(AntMazeEnv(maze_id=0, maze_size_scaling=4, maze_height=height,
    ...
    exp_prefix = 'test_height'
    now = datetime.datetime.now(dateutil.tz.tzlocal())
    timestamp = now.strftime('%Y_%m_%d_%H_%M_%S')
    exp_name = exp_prefix + '_{}MI_{}grid_{}latCat_bil_{:04d}_{}'.format(
        ''.join(str(snn_H_bonus).split('.')), mesh_density,
        latent_dim, s, timestamp)
