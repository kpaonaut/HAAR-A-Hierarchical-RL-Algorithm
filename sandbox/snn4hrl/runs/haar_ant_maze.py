...
import configs.example_config_for_no_transfer as par # import parameters
...
for maze_size_scaling in [4]:
    ...
    for time_step_agg in [10]:
        ...
        algo = TRPO(
            ...
            discount_low=0.99,
            discount_high=par.discount_high,
            train_high_every=par.train_high_every,
            step_size=0.01,
        ...
        for s in [40, 30, 20, 10, 0]:  # range(10, 110, 10):  # [10, 20, 30, 40, 50]:
            exp_prefix = par.exp_prefix_set
            now = datetime.datetime.now(dateutil.tz.tzlocal())
            timestamp = now.strftime('%Y_%m_%d_%H_%M_%S')

            exp_name = 'NoTransfer_{}mazeid_{}anneal_{}parallel_{}LowStepNum_{}MaxLowStep_{}timestepagg_{}'.format(
                int(par.maze_id),
