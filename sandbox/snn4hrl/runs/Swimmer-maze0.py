...
for dir in os.listdir(exp_dir):
    if 'Figure' not in dir and os.path.isfile(os.path.join(exp_dir, dir, 'params.pkl')):
        ...
        for maze_size_scaling in [4]:
            ...
            for time_step_agg in [50, 100]:

                inner_env = normalize(SwimmerMazeEnv(maze_id=0, maze_size_scaling=maze_size_scaling, ego_obs=True))
                ...
                algo = TRPO_snn(
                    ...
                    whole_paths=True,
                    # max_path_length=5e3 / time_step_agg * maze_size_scaling / 2.,  # correct for larger envs
