...
class CategoricalMLPPolicy(StochasticPolicy, LasagnePowered, Serializable):
    ...
    def get_action(self, observation):
        ...
        # for _ in range(10):
        #     print("OK!")
        # print(action) # see the shape of action! a single number!
        # action = 0 # fix action (latent) to zero!
    ...
    def get_params_snn(self): # Rui
        params = []
        for layer in self._layers:
            params += layer.get_params()
        return params

    def set_params_snn(self, snn_params): # Rui: reset params. copied from snn_mlp_policy_restorable.py
        if type(
