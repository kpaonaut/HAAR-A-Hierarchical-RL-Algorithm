...
def _worker_populate_task(G, env, policy, low_policy, scope=None):
    G = _get_scoped_G(G, scope)
    G.env = pickle.loads(env)
    # print('G.env: ', G.env.time_steps_agg)
    G.policy = pickle.loads(policy)
    if low_policy is not None:
        G.low_policy = pickle.loads(low_policy)
        G.env.low_policy = G.low_policy
...
def populate_task(env, policy, low_policy=None, scope=None):
    logger.log("Populating workers...")
    # print('env: ', env.time_steps_agg)
    # print('test:', pickle.loads(pickle.dumps(env)).time_steps_agg)
    if singleton_pool.n_parallel > 1:
        if low_policy is not None:
            singleton_pool.run_each(
                _worker_populate_task,
                [(pickle.dumps(env), pickle.dumps(policy), pickle.dumps(low_policy), scope)] * singleton_pool.n_parallel
            )
        else:
            singleton_pool.run_each(
                _worker_populate_task,
...
def _worker_set_low_policy_params(G, params, scope=None):
    G = _get_scoped_G(G, scope)
    G.low_policy.set_param_values(params)

def _worker_set_env_params(G, params, scope=None): # setting low policy params! (if any)
    G = _get_scoped_G(G, scope)
...
def sample_paths(
    ...
    if low_policy_params is not None:
        singleton_pool.run_each(
            _worker_set_low_policy_params,
            [(low_policy_params, scope)] * singleton_pool.n_parallel
        )
